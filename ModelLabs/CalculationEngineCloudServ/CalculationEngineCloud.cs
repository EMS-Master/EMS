...
namespace CalculationEngineCloudServ
{
    ...
    public class CalculationEngineCloud : ITransactionContract
    {
        ...
        public bool Optimize(List<MeasurementUnit> measEnergyConsumer, List<MeasurementUnit> measGenerators, float windSpeed, float sunlight)
        {
            ...
            if (InsertMeasurementsIntoDb(measurementsOptimized))
            {
                ...
                message = string.Format("Inserted {0} Measurement(s) into history database.", measGenerators.Count);
               // ServiceEventSource.Current.Message(message);
            }
        ...
        private Dictionary<long, OptimisationModel> CalculateWithGeneticAlgorithm(Dictionary<long, OptimisationModel> optModelMap, float powerOfConsumers)
        {
            ...
            CostAndProfitCalculationSfProxy costAndProfitCalculationSfProxy = new CostAndProfitCalculationSfProxy();
            ...
            if (gaoRenewable.NecessaryEnergy > 0)
            {
                ...
                foreach (var item in optModelMapOptimizied)
                {
                    if (optModelMap.ContainsKey(item.Key))
                    {
                        ...
                        string message = string.Format("Optimized production value for generator with GID: {0} => {1}\n", item.Key, item.Value.GenericOptimizedValue);
                        //ServiceEventSource.Current.Message(message);
                    }
                }
            }
            else
            {
                ...
                totalCost = costAndProfitCalculationSfProxy.CalculateTotalCostWhenNecessaryEnergyIsZero(commandedValues, totalCost);

                string message = string.Format("Necessary energy <= 0, no need for optimisation.\n");
                //ServiceEventSource.Current.Message(message);
            }
            ...
            profit = costAndProfitCalculationSfProxy.CalculateProfit(optModelMap, generators, allTypes, generatorCurves);
        ...
        public  bool SetAlgorithmParams(int iterationCount, int populationCount, int elitisamPct, float mutationRat)
        {
        ...
        public  bool SetAlgorithmParamsDefault()
        {
