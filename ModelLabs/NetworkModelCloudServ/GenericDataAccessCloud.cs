...
{
    public class GenericDataAccessCloud : INetworkModelGDAContract
    {
        private static Dictionary<int, ResourceIteratorCloud> resourceItMap = new Dictionary<int, ResourceIteratorCloud>();
        private static int resourceItId = 0;
        protected static NetworkModelCloud nm = null;

        public GenericDataAccessCloud()
        {
        }

        public static NetworkModelCloud NetworkModel
        {
            set
            {
                nm = value;
        ...
        public int GetExtentValues(ModelCode entityType, List<ModelCode> propIds)
        {
            try
            {
                ResourceIteratorCloud ri = nm.GetExtentValues(entityType, propIds);
        ...
        public int GetRelatedValues(long source, List<ModelCode> propIds, Association association)
        {
            try
            {
                ResourceIteratorCloud ri = nm.GetRelatedValues(source, propIds, association);
                int retVal = AddIterator(ri);
        ...
        private int AddIterator(ResourceIteratorCloud iterator)
        {
            lock (resourceItMap)
            {
        ...
        private ResourceIteratorCloud GetIterator(int iteratorId)
        {
            lock (resourceItMap)
            {
                ...
                {
                    //ServiceEventSource.Current.Message("Iterator with given ID = {0} doesn't exist.", iteratorId);
